...
if (isset($_POST['kirim'])) {
  ...
  if (isset($_SESSION['file_kk'])) {
    ...
    $file_kk = date('ymdhis') . $str1 . '.' . $ekstensi_kk;
    ...
    $sql = "INSERT INTO tb_bio_wni (user_id,file_kk,status_berkas,tgl,slug_kk,nmr_urut) VALUES ('$user_id','$_SESSION[file_kk]','$status','$tgl_waktu','$_SESSION[slug_kk]','$nmr_urut')";
  } else {
    $str = rand();
    $str1 = md5($str);
    ...
    //Query input menginput data kedalam tabel kk
    $sql = "INSERT INTO tb_bio_wni (user_id,file_kk,status_berkas,tgl,slug_kk,nmr_urut) VALUES ('$user_id','$file_kk','$status','$tgl_waktu','$file_kk1','$nmr_urut')";
  }
...
if (isset($_POST['ubah']) and $baru === 'Baru') {
  ...
  $ekstensi_KK = strtolower(end($pdf_KK));
  $namaFileKK = date('ymdhis') . $str1 . '.' . $ekstensi_KK;
